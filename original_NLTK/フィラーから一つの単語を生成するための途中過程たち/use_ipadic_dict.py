...
def build_lattice(text, dictionary):
    ...
    for start, nodes in lattice.items():
        node_info = [(node.word, node.read, node.cost) for node in nodes]
        print(f"Start position {start}: {node_info}")
...
def dijkstra_lattice(lattice, text_len):
    ...
    print("dijkstra structure:")
    for position, node in previous_nodes.items():
        if node is not None:
            print(f"Position {position}: Word='{node.word}', Read='{node.read}', Cost={node.cost}")
        else:
            print(f"Position {position}: None")
